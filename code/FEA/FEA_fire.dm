...
/turf/simulated/hotspot_expose(exposed_temperature, exposed_volume, soh)
	...
	if(active_hotspot)
		if(soh)
			if(air_contents.phoron > 0.5 && air_contents.oxygen > 0.5)
	...
	if((exposed_temperature > PLASMA_MINIMUM_BURN_TEMPERATURE) && air_contents.phoron > 0.5)
		igniting = 1

	if(igniting)
		if(air_contents.oxygen < 0.5 || air_contents.phoron < 0.5)
			return 0
...
/obj/effect/hotspot/process(turf/simulated/list/possible_spread)
	...
	if(location.air.phoron < 0.5 || location.air.oxygen < 0.5)
		Kill()
		return
