...
obj/item/weapon/tank
	verb
		adjust_mixture(temperature as num, target_toxin_pressure as num, target_oxygen_pressure as num)
			...
			air_contents.phoron = target_toxin_pressure*air_contents.volume/(R_IDEAL_GAS_EQUATION*air_contents.temperature)
		...
		verb
			...
			pipeline_data()
				...
				if(parent)
					...
					usr << "[parent.air.oxygen], [parent.air.phoron], [parent.air.nitrogen], [parent.air.carbon_dioxide] .. [parent.alert_pressure]"
*/
mob
	verb
...
turf/simulated
	...
	verb
		...
		air_status()
			...
			usr << "\blue @[x],[y] ([GM.group_multiplier]): O:[GM.oxygen] T:[GM.phoron] N:[GM.nitrogen] C:[GM.carbon_dioxide] w [GM.temperature] Kelvin, [GM.return_pressure()] kPa [(active_hotspot)?("\red BURNING"):(null)]"
	...
	proc/measurement()
		...
		switch(measure)
			...
			if("plasma")
				if(GM.phoron < 0)
					return "error"
				return "[round(GM.phoron/MOLES_CELLSTANDARD*10+0.5)]"
			if("nitrogen")
				if(GM.nitrogen < 0)
					return "error"
