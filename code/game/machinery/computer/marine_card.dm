...
/obj/machinery/computer/marine_card/attack_hand(var/mob/user as mob)
	...
	if (mode) // accessing crew manifest
		...
		if (authenticated && modify)
			...
			if(istype(src,/obj/machinery/computer/card/centcom))
				...
				accesses += "<tr>"
				for(var/i = 1; i <= 7; i++)
					accesses += "<td style='width:14%'><b>[get_marine_region_accesses_name(i)]:</b></td>"
				accesses += "</tr><tr>"
				for(var/i = 1; i <= 8; i++)
					accesses += "<td style='width:14%' valign='top'>"
					for(var/A in get_marine_region_accesses(i))
						if(A in modify.access)
							accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=0'><font color=\"red\">[replacetext(get_marine_access_desc(A), " ", "&nbsp")]</font></a> "
						else
							accesses += "<a href='?src=\ref[src];choice=access;access_target=[A];allowed=1'>[replacetext(get_marine_access_desc(A), " ", "&nbsp")]</a> "
						accesses += "<br>"
					accesses += "</td>"
				accesses += "</tr></table>"
	...
	user << browse(dat, "window=id_com;size=1184x850")
	onclose(user, "id_com")
	return
