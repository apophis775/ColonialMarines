...
/obj/structure/table/reinforced/attack_alien()
	if (islarva(usr))//Safety check for larva. /N
		return

	usr << "\red You claw at the [name]."
	for(var/mob/O in oviewers(src))
		O.show_message("\red [usr] claws at the [name]!", 1)
	//playsound(loc, 'sound/effects/attackblob.ogg', 100, 1)
	health -= rand(40, 60)
	if(health <= 0)
		usr << "\red You slice the [name] to pieces."
		for(var/mob/O in oviewers(src))
			O.show_message("\red [usr] slices the [name] apart!", 1)
		del(src)
	return

/obj/structure/table/reinforced/proc/healthcheck()
	if(health <= 0)
		density = 0
		del(src)
	return
