			...
			if (3)
				...
				if (isnull(T))
					...
					if (total_moles)
						...
						var/plasma_level = environment.phoron/total_moles
		...
		if(5)
			if((istype(A, /obj/item/weapon/tank)) || (istype(A, /obj/machinery/portable_atmospherics)))
				...
				if (total_moles>0)
					...
					var/plasma_concentration = A:air_contents.phoron/total_moles
			...
			if (istype(A, /obj/machinery/atmospherics/pipe/tank))
				...
				if (total_moles>0)
					...
					var/plasma_concentration = T.parent.air.phoron/total_moles
