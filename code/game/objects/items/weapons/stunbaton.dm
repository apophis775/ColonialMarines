...
/obj/item/weapon/melee/baton/attack(mob/M as mob, mob/user as mob)
	...
	if(user.a_intent == "hurt")
		if(!..()) return
		C.visible_message("<span class='danger'>[M] has been beaten with the [src] by [user]!</span>")

		user.attack_log += "\[[time_stamp()]\]<font color='red'> Beat [C.name] ([C.ckey]) with [src.name]</font>"
		C.attack_log += "\[[time_stamp()]\]<font color='orange'> Beaten by [user.name] ([user.ckey]) with [src.name]</font>"
		msg_admin_attack("[user.name] ([user.ckey]) beat [C.name] ([C.ckey]) with [src.name] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")

		playsound(src.loc, "swing_hit", 50, 1, -1)
	else if(!status)
		C.visible_message("<span class='warning'>[M] has been prodded with the [src] by [user]. Luckily it was off.</span>")
		return
	...
	if(status)
		...
		if(isalien(C))
			...
			switch(T.class)
				if(0) //Tier 0
					C.weakened = rand(8,12)
					C.visible_message("<span class='danger'>[M] has been prodded with the [src] by [user]!</span>")
					//--Logs
					user.attack_log += "\[[time_stamp()]\]<font color='red'> Stunned [C.name] ([C.ckey]) with [src.name]</font>"
					C.attack_log += "\[[time_stamp()]\]<font color='orange'> Stunned by [user.name] ([user.ckey]) with [src.name]</font>"
					log_attack("[user.name] ([user.ckey]) stunned [C.name] ([C.ckey]) with [src.name]")
				...
				if(1) //Tier I
					if(prob(60))
						...
						C.visible_message("<span class='danger'>[M] has been prodded with the [src] by [user], but it is still standing!</span>")
