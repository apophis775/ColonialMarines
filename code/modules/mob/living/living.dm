...
/mob/living/verb/resist()
	...
	//breaking out of handcuffs
		...
		if(CM.handcuffed && CM.canmove && (CM.last_special <= world.time))
			...
			if(istype(CM, /mob/living/carbon/alien))
				for(var/mob/O in viewers(CM))
					O.show_message(text("\red <b>[] spits acid on the restraints!</b>", CM), 1)
				spawn(0)
					if(do_after(CM, 300)) // After 300 seconds
						if(!CM.handcuffed || CM.buckled)
							return
						for(var/mob/O in viewers(CM))
							O.show_message(text("\red <B>[]'s restraints melt under the acid!</B>", CM), 1)
						CM << "\red You successfully melt your restraints."
						del(CM.handcuffed)
						CM.handcuffed = null
