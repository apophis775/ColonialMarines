...
/obj/item/clothing/mask/facehugger/proc/Attach(M as mob)
	...
	if(iscarbon(M))
		...
		target.visible_message("\red \b [src] is trying to latch onto [target]'s face!")
		spawn(70)
			var/obj/item/clothing/mask/facehugger/hugger = target.wear_mask
			if(istype(hugger) && hugger.loc == target)
				target.visible_message("\red \b [src] has latched onto [target]'s face!")
				src.canremove = 0
