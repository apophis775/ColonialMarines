...
/client/proc/cmd_admin_check_new_players()	//Allows mentors / admins to determine who the newer players are.
	set category = "Admin"
...
/client/proc/cmd_admin_local_narrate()	// local narrate -- TLE
	set category = "Special Verbs"
	...
	var/msg = input("Message:", text("Enter the text you wish to appear to everyone in view:")) as text

	if( !msg )
		return
	for(var/mob/living/M in view())
		M << msg
	log_admin("LocalNarrate: [key_name(usr)] narrated : [msg]")
	message_admins("\blue \bold LocalNarrate: [key_name(usr)] narrated : [msg]<BR>", 1)
	feedback_add_details("admin_verb","LON") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/respawn_character()
	...
	if(!new_character.mind.assigned_role)
		new_character.mind.assigned_role = "Marine"//If they somehow got a null assigned role.
...
/client/proc/cmd_admin_kill(mob/O in living_mob_list)
	set category = "Admin"
	set name = "Kill"
	...
	if (alert(src, "Are you sure you want to kill:\n[O]\nat ([O.x], [O.y], [O.z])?", "Confirmation", "Yes", "No") == "Yes")
		log_admin("[key_name(usr)] killed [O] at ([O.x],[O.y],[O.z])")
		message_admins("[key_name_admin(usr)] killed [O] at ([O.x],[O.y],[O.z])", 1)
		feedback_add_details("admin_verb","DEL") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		O.death()
		O.stat = DEAD
