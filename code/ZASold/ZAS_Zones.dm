...
/zone/New(turf/start)
	...
	for(var/turf/simulated/T in contents)
		...
		air.phoron += T.phoron / air.group_multiplier
...
/zone/proc/UpdateUnsimAvg()
	...
	air_unsim.phoron *= correction_ratio
...
/zone/proc/process()
	...
	air.graphic = 0
	if(air.phoron > MOLES_PLASMA_VISIBLE)
		air.graphic = 1
	...
	if(length(connections))
		...
		for(var/zone/Z in closed_connection_zones)
			...
			var/handle_temperature
			if(air)	//Hopefully fix a runetime error
				handle_temperature = abs(air.temperature - Z.air.temperature) > vsc.connection_temperature_delta
...
proc/ShareRatio(datum/gas_mixture/A, datum/gas_mixture/B, connecting_tiles)
	...
	var
		...
		s_full_plasma = B.phoron * share_size
	...
	A.phoron = max(0, (A.phoron - plasma_avg) * (1-ratio) + plasma_avg )
	...
	B.phoron = max(0, (B.phoron - plasma_avg) * (1-ratio) + plasma_avg )
