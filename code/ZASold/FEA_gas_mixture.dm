...
/datum/gas_mixture/proc/check_then_merge(datum/gas_mixture/giver)
	...
	if(((giver.oxygen > MINIMUM_AIR_TO_SUSPEND) && (giver.oxygen >= oxygen*MINIMUM_AIR_RATIO_TO_SUSPEND)) \
		...
		|| ((giver.phoron > MINIMUM_AIR_TO_SUSPEND) && (giver.phoron >= toxins*MINIMUM_AIR_RATIO_TO_SUSPEND)))
		return 0
...
/datum/gas_mixture/proc/merge(datum/gas_mixture/giver)
	...
	if((group_multiplier>1)||(giver.group_multiplier>1))
		...
		toxins += giver.phoron*giver.group_multiplier/group_multiplier
	else
		oxygen += giver.oxygen
...
/datum/gas_mixture/proc/remove(amount)
	...
	removed.phoron = QUANTIZE(((toxins/sum)*amount))
	...
	toxins -= removed.phoron/group_multiplier
...
/datum/gas_mixture/proc/remove_ratio(ratio)
	...
	removed.phoron = QUANTIZE(toxins*ratio)
...
/datum/gas_mixture/proc/check_gas_mixture(datum/gas_mixture/sharer)
	...
	var/delta_toxins = QUANTIZE(toxins_archived - sharer.phoron_archived)/TRANSFER_FRACTION
	...
	if(((abs(delta_oxygen) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_oxygen) >= sharer.oxygen_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)) \
		...
		|| ((abs(delta_toxins) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_toxins) >= sharer.phoron_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)))
		return -1
...
/datum/gas_mixture/proc/check_turf(turf/model)
	...
	var/delta_toxins = (toxins_archived - model.phoron)/TRANSFER_FRACTION
...
/datum/gas_mixture/proc/share(datum/gas_mixture/sharer)
	...
	sharer.phoron += delta_toxins/sharer.group_multiplier
...
/datum/gas_mixture/proc/mimic(turf/model, border_multiplier)
	...
	var/delta_toxins = QUANTIZE(toxins_archived - model.phoron)/TRANSFER_FRACTION
...
/datum/gas_mixture/proc/compare(datum/gas_mixture/sample)
	...
	if((abs(carbon_dioxide-sample.carbon_dioxide) > MINIMUM_AIR_TO_SUSPEND) && \
		...
		return 0
	if((abs(toxins-sample.phoron) > MINIMUM_AIR_TO_SUSPEND) && \
		((toxins < (1-MINIMUM_AIR_RATIO_TO_SUSPEND)*sample.phoron) || (toxins > (1+MINIMUM_AIR_RATIO_TO_SUSPEND)*sample.phoron)))
		return 0
...
/datum/gas_mixture/proc/subtract(datum/gas_mixture/right_side)
	...
	toxins = max(toxins - right_side.phoron)
