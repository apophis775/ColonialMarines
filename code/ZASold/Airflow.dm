...
//		M.show_message("\red <B>[src] slams into [A]!</B>",1,"\red You hear a loud slam!",2)
	playsound(src.loc, "punch3.ogg", 25, 1, -1)
	loc:add_blood(src)
	if (src.wear_suit)
	...
	apply_damage(b_loss/1, BRUTE, "head", blocked, 0, "Airflow")

	blocked = run_armor_check("chest","melee")
	apply_damage(b_loss/1, BRUTE, "chest", blocked, 0, "Airflow")

	blocked = run_armor_check("groin","melee")
	apply_damage(b_loss/1, BRUTE, "groin", blocked, 0, "Airflow")

	if(airflow_speed > 10)
		paralysis += round(airflow_speed * vsc.airflow_stun)
		weakened = max(weakened,5)
	else
		weakened += round(airflow_speed * vsc.airflow_stun/2)
	. = ..()

mob/living/carbon/alien/airflow_hit(atom/A)
//	for(var/mob/M in hearers(src))
//		M.show_message("\red <B>[src] slams into [A]!</B>",1,"\red You hear a loud slam!",2)
	playsound(src.loc, "punch3.ogg", 25, 1, -1)
	loc:add_blood(src)
